<?php namespace Ifsnop\Mysqldump;
    ...
    */
    ...
    private function parseDsn($dsn)
    {
        if (empty($dsn) || (false === ($pos = strpos($dsn, ":")))) {
            throw new Exception(__("Empty DSN string",'wpkbd'));
        }
        ...
        if (empty($this->dbType)) {
            throw new Exception(__("Missing database type from DSN string",'wpkbd'));
        }
        ...
        if (empty($this->dsnArray['host']) &&
            ...
            throw new Exception(__("Missing host from DSN string",'wpkbd'));
        }
        ...
        if (empty($this->dsnArray['dbname'])) {
            throw new Exception(__("Missing database name from DSN string",'wpkbd'));
        }
...
class CompressBzip2 extends CompressManagerFactory
{
    ...
    public function __construct()
    {
        if (! function_exists("bzopen")) {
            throw new Exception(__("Compression is enabled, but bzip2 lib is not installed or configured properly",'wpkbd'));
        }
    }
    ...
    public function open($filename)
    {
        ...
        if (false === $this->fileHandler) {
            throw new Exception(__("Output file is not writable",'wpkbd'));
        }
    ...
    public function write($str)
    {
        if (false === ($bytesWritten = bzwrite($this->fileHandler, $str))) {
            throw new Exception(__("Writting to file failed! Probably, there is no more free space left?",'wpkbd'));
        }
        return $bytesWritten;
    }
...
class CompressGzip extends CompressManagerFactory
{
    ...
    public function __construct()
    {
        if (! function_exists("gzopen")) {
            throw new Exception(__("Compression is enabled, but",'wpkbd')." gzip lib ".__("is not installed or configured properly",'wpkbd'));
        }
    }
...
abstract class TypeAdapterFactory
{
    ...
    public static function create($c, $dbHandler = null)
    {
        ...
        if (! TypeAdapter::isValid($c)) {
            throw new Exception(__("Database type support for",'wpkbd')." ($c) ".__("not yet available",'wpkbd'));
        }
...
class TypeAdapterMysql extends TypeAdapterFactory
{
    ...
    public function create_view($row)
    {
        ...
        if (!isset($row['Create View'])) {
                throw new Exception(__("Error getting view structure, unknown output",'wpkbd'));
        }
    ...
    public function create_trigger($row)
    {
        ...
        if (!isset($row['SQL Original Statement'])) {
            throw new Exception(__("Error getting trigger code, unknown output",'wpkbd'));
        }
    ...
    public function create_procedure($row, $dumpSettings)
    {
        ...
        if (!isset($row['Create Procedure'])) {
            throw new Exception(__("Error getting procedure code, unknown output.",'wpkbd') ." ".
                __("Please check",'wpkbd')." 'https://bugs.mysql.com/bug.php?id=14564'");
        }
    ...
    public function create_event($row)
    {
        ...
        if ( !isset($row['Create Event']) ) {
            throw new Exception(__("Error getting event code, unknown output.",'wpkbd') ." ".
                __("Please check",'wpkbd')." 'http://stackoverflow.com/questions/10853826/mysql-5-5-create-event-gives-syntax-error'");
        }
        $eventName = $row['Event'];
    ...
    private function check_parameters($num_args, $expected_num_args, $method_name)
    {
        if ( $num_args != $expected_num_args ) {
            throw new Exception(__("Unexpected parameter passed to",'wpkbd')." $method_name");
        }
        return;
    }
}
