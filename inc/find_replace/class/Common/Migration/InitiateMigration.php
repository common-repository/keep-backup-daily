...
class InitiateMigration {
	...
	function ajax_initiate_migration() {
		...
		update_site_option( 'kbfnr_usage', array( 'action' => $state_data['intent'], 'time' => time() ) );
		...
		if ( in_array( $state_data['intent'], array( 'find_replace', 'savefile', 'import' ) ) ) {
			...
			$data = array(
				'action'       => 'kbfnr_remote_initiate_migration',
			...
			if ( false === $return ) {
				$error_msg = __( 'Failed attempting to unserialize the response from the remote server. Please contact support.', 'wpkbd' );
				$return    = array( 'kbfnr_error' => 1, 'body' => $error_msg );
			...
			if ( isset( $return['error'] ) && $return['error'] == 1 ) {
				$return = array( 'kbfnr_error' => 1, 'body' => $return['message'] );
				$result = $this->http->end_ajax( json_encode( $return ) );
		...
		// A successful call to kbfnr_remote_initiate_migration for a Push migration will have set db_version.
		...
		// A successful call to kbfnr_remote_initiate_migration for a Push migration will have set site_url.
...
//		echo json_encode($state);exit;
		...
		do_action( 'kbfnr_initiate_migration', $state_data );
	...
	function parse_find_replace_pairs( $intent = '', $site_url = '' ) {
		...
		$tmp_find_replace_pairs = apply_filters( 'kbfnr_find_and_replace', $tmp_find_replace_pairs, $intent, $site_url );
