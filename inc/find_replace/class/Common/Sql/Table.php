...
class Table {
	...
	public function __construct(
		...
		$this->rows_per_segment = apply_filters( 'kbfnr_rows_per_segment', 100 );
	 ...
	 * @return mixed
	 *
	 * NOTE: Returned array may have been altered by kbfnr_table_sizes filter.
	 */
	function get_table_sizes() {
		...
		return apply_filters( 'kbfnr_table_sizes', $return, 'regular' );
	}
	...
	function get_legacy_alter_table_name() {
		...
		global $wpdb;
		$alter_table_name = apply_filters( 'kbfnr_alter_table_name', $wpdb->base_prefix . 'kbfnr_alter_statements' );
	...
	function get_preserved_options_queries( $temp_tables, $intent = '' ) {
		...
		if ( is_multisite() ) {
			...
			if ( ! empty( $sitemeta_table_name ) ) {
				...
				$preserved_sitemeta_options         = apply_filters( 'kbfnr_preserved_sitemeta_options', $preserved_sitemeta_options, $intent );
				...
				$preserved_sitemeta_options_data = apply_filters( 'kbfnr_preserved_sitemeta_options_data', $preserved_sitemeta_options_data, $intent );
		...
		if ( ! empty( $options_table_names ) ) {
			$preserved_options         = apply_filters( 'kbfnr_preserved_options', $preserved_options, $intent );
			...
			$preserved_options_data = apply_filters( 'kbfnr_preserved_options_data', $preserved_options_data, $intent );
	...
	/**
	 * Preserves kbfnr plugins if the "Keep active plugins" option isn't checked.
	 *
	 * @param array $preserved_options_data
	 *
	 * @return array
	 */
	function preserve_kbfnr_plugins( $preserved_options_data ) {
		$state_data          = $this->migration_state_manager->set_post_data();
		$keep_active_plugins = $this->util->profile_value( 'keep_active_plugins', $this->form_data->parse_migration_form_data( $state_data['form_data'] ) );
			...
			foreach ( $data as $key => $option ) {
				if ( 'active_plugins' === $option['option_name'] ) {
					...
					foreach ( $option_value as $plugin_key => $plugin ) {
						if ( 0 === strpos( $plugin, 'wpkbd' ) ) {
							$kbfnr_plugins[] = $plugin;
						}
					}

					$merged_plugins                           = array_unique( array_merge( $kbfnr_plugins, $migrated_plugins ) );
	...
	function process_table( $table, $fp = null ) {
		...
		$target_table_name = apply_filters( 'kbfnr_target_table_name', $table, $form_data['action'], $state_data['stage'], $site_details );
	...
	function get_structure_info( $table, $table_structure = array() ) {
		...
		foreach ( $table_structure as $struct ) {
			...
			} elseif ( 0 === strpos( $struct->Type, 'binary' ) || apply_filters( 'kbfnr_process_column_as_binary', false, $struct ) ) {
				$bins[ strtolower( $struct->Field ) ] = '1';
			} elseif ( 0 === strpos( $struct->Type, 'bit' ) || apply_filters( 'kbfnr_process_column_as_bit', false, $struct ) ) {
	...
	function get_table_structure( $table ) {
		...
		if ( ! $table_structure ) {
			$this->error_log->setError( sprintf( __( 'Failed to retrieve table structure for table', 'wpkbd' ).' \'%s\', '.__('please ensure your database is online.', 'wpkbd' ).' (#125)', $table ) );
	...
	function pre_process_data( $table, $target_table_name, $temp_table_name, $fp ) {
		...
		if ( in_array( $form_data['action'], array( 'find_replace', 'import' ) ) ) {
			if ( 'backup' === $state_data['stage'] ) {
				...
				if ( true !== $create ) {
					$message = sprintf( __( 'Error creating temporary table. Table', 'wpkbd' ).' "%s" '.__('does not exist.', 'wpkbd' ), esc_html( $table ) );

					return $this->http->end_ajax( json_encode( array( 'kbfnr_error', 1, 'body' => $message ) ) );
				}
			}
		} else {
			$this->build_table_header( $table, $target_table_name, $temp_table_name, $fp );
		 ...
		 * @param string $temp_table_name
		 */
		do_action( 'kbfnr_pre_process_table_data', $table, $target_table_name, $temp_table_name );
	}
	...
	function build_table_header( $table, $target_table_name, $temp_table_name, $fp ) {
		...
		// Don't stow data until after `kbfnr_create_table_query` filter is applied as mysql_compat_filter() can return an error
		$stow          = '';
		$is_backup     = false;
		...
		if ( $is_backup ) {
			$stow .= ( "\n\n" );
			$stow .= ( "#\n" );
			$stow .= ( '# ' . sprintf( __( 'Delete any existing table', 'wpkbd' ).' %s', $this->table_helper->backquote( $table_to_stow ) ) . "\n" );
			$stow .= ( "#\n" );
			$stow .= ( "\n" );
		}
		...
		if ( $is_backup ) {
			$stow .= ( "\n\n" );
			$stow .= ( "#\n" );
			$stow .= ( '# ' . sprintf( __( 'Table structure of table', 'wpkbd' ).' %s', $this->table_helper->backquote( $table_to_stow ) ) . "\n" );
			$stow .= ( "#\n" );
			$stow .= ( "\n" );
		}
		...
		if ( false === $create_table ) {
			$this->error_log->setError( __( 'Failed to generate the create table query, please ensure your database is online.', 'wpkbd' ).' (#126)' );
		...
		$create_table[0][1] = apply_filters( 'kbfnr_create_table_query', $create_table[0][1], $table_to_stow, $this->dynamic_props->target_db_version, $form_data['action'], $state_data['stage'] );
		...
		$alter_data_queries = apply_filters( 'kbfnr_alter_data_queries', $alter_data_queries, $table_to_stow, $form_data['action'], $state_data['stage'] );
		...
		if ( $is_backup ) {
			$this->stow( "\n\n", false, $fp );
			$this->stow( "#\n", false, $fp );
			$this->stow( '# ' . sprintf( __( 'Data contents of table', 'wpkbd' ).' %s', $this->table_helper->backquote( $table_to_stow ) ) . "\n", false, $fp );
			$this->stow( "#\n", false, $fp );
		}
	}
		...
		if ( 'savefile' === $form_data['action'] || in_array( $state_data['stage'], array( 'backup', 'import' ) ) ) {
			if ( Util::gzip() && isset( $form_data['gzip_file'] ) && $form_data['gzip_file'] ) {
				if ( ! gzwrite( $fp, $query_line ) ) {
					$this->error_log->setError( __( 'Failed to write the gzipped SQL data to the file.', 'wpkbd' ).' (#127)' );
				...
				if ( false === @fwrite( $fp, $query_line ) ) {
					$this->error_log->setError( __( 'Failed to write the SQL data to the file.', 'wpkbd' ).' (#128)');

					return false;
				}
			}
		} elseif ( $state_data['intent'] == 'pull' ) {
			echo apply_filters( 'kbfnr_before_response', $query_line );
	...
	function process_chunk( $chunk ) {
		...
		foreach ( $queries as $query ) {
			if ( false === $wpdb->query( $query ) ) {
				$return = ob_get_clean();
				$return = array( 'kbfnr_error' => 1, 'body' => $return );
	...
	/**
	 * Check if query failed due to invalid text and retry stripped query if kbfnr_STRIP_INVALID is defined as true
	 *
	 * @param string $query
	...
	function maybe_strip_invalid_text_and_retry( $query, $context = 'default' ) {
		...
		if ( ! $wpdb->last_error && ( strstr( $query, 'INSERT' ) || strstr( $query, 'UPDATE' ) ) ) {
			// Only instantiate kbfnr_WPDB if needed
			if ( ! $this->wpdb ) {
				$this->wpdb = Util::make_kbfnr_wpdb_instance();
			}
			if ( $this->wpdb->query_has_invalid_text( $query ) ) {
				if ( ! ( defined( 'kbfnr_STRIP_INVALID_TEXT' ) && kbfnr_STRIP_INVALID_TEXT ) ) {
					...
					if ( false === $wpdb->query( $this->wpdb->last_stripped_query ) ) {
						...
						$return = new \WP_Error( 'strip_invalid_text_query_failed', __('Failed to import the stripped SQL query: ', 'wpkbd') . $error );
					} else {
						$return = true;
					}
				}
	...
	function get_alter_table_name() {
		...
		$alter_table_name = apply_filters( 'kbfnr_alter_table_name', $this->props->temp_prefix . 'kbfnr_alter_statements' );
	...
	function build_select_query( $table, $row_start, $structure_info ) {
		...
		if ( 'import' !== $state_data['intent'] && 'backup' != $state_data['stage'] && true === apply_filters( 'kbfnr_exclude_transients', true ) && isset( $form_data['exclude_transients'] ) && '1' === $form_data['exclude_transients'] && ( $this->table_helper->table_is( 'options', $table, 'table', $prefix ) || ( isset( $wpdb->sitemeta ) && $wpdb->sitemeta == $table ) ) ) {
		...
		// don't export/migrate kbfnr specific option rows unless we're performing a backup
		if ( 'backup' != $state_data['stage'] && ( $this->table_helper->table_is( 'options', $table, 'table', $prefix ) || ( isset( $wpdb->sitemeta ) && $wpdb->sitemeta == $table ) ) ) {
			...
			$where .= " AND `{$col_name}` != 'kbfnr_settings'";
			$where .= " AND `{$col_name}` != 'kbfnr_error_log'";
			$where .= " AND `{$col_name}` != 'kbfnr_file_ignores'";
			$where .= " AND `{$col_name}` != 'kbfnr_schema_version'";
			$where .= " AND `{$col_name}` NOT LIKE 'kbfnr_state_%'";
		}
		...
		$join     = apply_filters( 'kbfnr_rows_join', $join, $table );
		$where    = apply_filters( 'kbfnr_rows_where', $where, $table );
		$order_by = apply_filters( 'kbfnr_rows_order_by', $order_by, $table );
		$limit    = apply_filters( 'kbfnr_rows_limit', $limit, $table );

		$sql = 'SELECT ' . $sel . ' FROM ' . $this->table_helper->backquote( $table ) . " $join $where $order_by $limit";
		$sql = apply_filters( 'kbfnr_rows_sql', $sql, $table );
	...
	function process_row( $table, $replacer, $row, $structure_info, $fp ) {
		...
		if ( ! apply_filters( 'kbfnr_table_row', $row, $table, $form_data['action'], $state_data['stage'] ) ) {
			$skip_row = true;
		}
				...
				if ( is_multisite() && in_array( $table, array( $wpdb->site, $wpdb->blogs, $this->props->temp_prefix . $wpdb->blogs, $this->props->temp_prefix . $wpdb->site ) ) ) {
					...
					if ( 'backup' !== $state_data['stage'] ) {
						...
						if ( 'path' == $key ) {
							...
							$new_path_current_site = apply_filters( 'kbfnr_new_path_current_site', $new_path_current_site );
						...
						if ( 'domain' == $key ) { // wp_blogs and wp_sites tables
							...
							$domain_replaces = apply_filters( 'kbfnr_domain_replaces', $domain_replaces );
	...
	function transfer_chunk( $fp ) {
		...
		if ( false === empty( $maybe_errors ) ) {
			$maybe_errors = array( 'kbfnr_error' => 1, 'body' => $maybe_errors );
		...
		if ( '1' !== $response ) {
			...
			if ( $decoded_response && isset( $decoded_response['kbfnr_error'] ) && isset( $decoded_response['body'] ) ) {
				// $response is already json_encoded kbfnr_error object
	...
	function post_process_data( $table, $target_table_name, $fp ) {
		 ...
		 * @param string $target_table_name
		 */
		do_action( 'kbfnr_post_process_table_data', $table, $target_table_name );
	...
	function build_table_footer( $table, $target_table_name, $fp ) {
		...
		$stow = "\n";
		$stow .= "#\n";
		$stow .= '# ' . sprintf( __( 'End of data contents of table', 'wpkbd' ).' %s', $this->table_helper->backquote( $target_table_name ) ) . "\n";
		$stow .= "# --------------------------------------------------------\n";
		$stow .= "\n";
		$this->stow( $stow, false, $fp );
	...
	function get_alter_queries() {
		...
		if ( $alter_table_name === $wpdb->get_var( "SHOW TABLES LIKE '$alter_table_name'" ) ) {
			...
			$alter_queries = apply_filters( 'kbfnr_get_alter_queries', $alter_queries );
		}
	...
	function get_create_alter_table_query() {
		...
		$this->create_alter_table_query = apply_filters( 'kbfnr_create_alter_table_query', $this->create_alter_table_query );
	...
	/**
	 * Get only the tables beginning with our DB prefix or temporary prefix, also skip views and legacy kbfnr_alter_statements table.
	 *
	 * @param string $scope
	 *
	 * @return array
	...
	function get_tables( $scope = 'regular' ) {
		...
		return apply_filters( 'kbfnr_tables', $clean_tables, $scope );
	}
	...
	function db_backup_header( $fp ) {
		...
		$charset = ( defined( 'DB_CHARSET' ) ? DB_CHARSET : 'utf8' );
		$this->stow( '# ' . __( 'WordPress MySQL database migration', 'wpkbd' ) . "\n", false, $fp );
		$this->stow( "#\n", false, $fp );
		$this->stow( '# ' . sprintf( __( 'Generated', 'wpkbd' ).': %s', date( 'l j. F Y H:i T' ) ) . "\n", false, $fp );
		$this->stow( '# ' . sprintf( __( 'Hostname', 'wpkbd' ).': %s', DB_HOST ) . "\n", false, $fp );
		$this->stow( '# ' . sprintf( __( 'Database', 'wpkbd' ).': %s', $this->table_helper->backquote( DB_NAME ) ) . "\n", false, $fp );

		$home_url = apply_filters( 'kbfnr_backup_header_url', home_url() );
		...
		$included_tables = apply_filters( 'kbfnr_backup_header_included_tables', $included_tables );
		...
		$is_subsite_export = apply_filters( 'kbfnr_backup_header_is_subsite_export', 'false' );
	...
	function get_post_types() {
		...
		return apply_filters( 'kbfnr_post_types', array_unique( $return ) );
	}
