...
namespace WPMechanic\KBFNR\Common\Error;
...
class ErrorLog {
	...
	function load_error_log() {
		$this->error_log = get_site_option( 'kbfnr_error_log' );
		...
		if ( false === $this->error_log && is_multisite() && is_network_admin() ) {
			$this->error_log = get_option( 'kbfnr_error_log' );
			if ( false !== $this->error_log ) {
				update_site_option( 'kbfnr_error_log', $this->error_log );
	...
	function log_error( $kbfnr_error, $additional_error_var = false ) {
		...
		$max_log_length            = apply_filters( 'kbfnr_max_error_log_length', 1000000 );
		$max_individual_log_length = apply_filters( 'kbfnr_max_individual_error_log_length', $max_log_length / 2.2 );
	...
	/**
	 * Check for kbfnr-download-log and related nonce
	 * if found begin diagnostic logging
	 *
	 * @return void
	 */
	function http_prepare_download_log() {
		if ( isset( $_GET['kbfnr-download-log'] ) && wp_verify_nonce( $_GET['nonce'], 'kbfnr-download-log' ) ) {
			ob_start();
	...
	function get_diagnostic_info() {
		...
		$mdb_plugins = array();
		foreach ( array_reverse( $GLOBALS['kbfnr_meta'] ) as $kbfnr_plugin => $kbfnr_plugin_info ) {
			if ( strlen( $kbfnr_plugin ) > strlen( '' ) ) {
				$kbfnr_plugin = str_replace( '-', '', $kbfnr_plugin );
			}
			$kbfnr_plugin                 = ucwords( str_replace( array( 'wp', 'db', 'cli', '-' ), array(
				'WP',
				'DB',
				'CLI',
				' ',
			), $kbfnr_plugin ) );
			$mdb_plugins[ $kbfnr_plugin ] = $kbfnr_plugin_info['version'];
		}
		...
		$diagnostic_info['db-server-info'] = array(
			...
			'kbfnr_STRIP_INVALID_TEXT' => ( defined( 'kbfnr_STRIP_INVALID_TEXT' ) && kbfnr_STRIP_INVALID_TEXT ) ? 'Yes' : 'No',
		);
		...
		$diagnostic_info['mdb-settings'] = array(
			'kbfnr Bottleneck'       => size_format( $this->util->get_bottleneck() ),
			'Compatibility Mode'     => ( isset( $GLOBALS['kbfnr_compatibility']['active'] ) ) ? 'Yes' : 'No',
		...
		$diagnostic_info = array_merge( $diagnostic_info, apply_filters( 'kbfnr_diagnostic_info', array(), $diagnostic_info ) );
		...
		if ( isset( $GLOBALS['kbfnr_compatibility']['active'] ) ) {
