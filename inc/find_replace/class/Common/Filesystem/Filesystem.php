...
namespace WPMechanic\KBFNR\Common\Filesystem;
...
class Filesystem {
	...
	private $chmod_file;
	/**
	 * @var \WPMechanic\KBFNR\League\Container\Container|null
	 */
	private $container;
	...
	public function check_for_wp_filesystem() {
		if ( function_exists( 'request_filesystem_credentials' ) ) {
			if ( ( defined( 'kbfnr_WP_FILESYSTEM' ) && kbfnr_WP_FILESYSTEM ) || ! defined( 'kbfnr_WP_FILESYSTEM' ) ) {
	...
	function download_file() {
		...
		if ( file_exists( $diskfile ) ) {
			if ( ! headers_sent() ) {
				...
				wp_die( sprintf( '<h3>'.__('Output prevented download.', 'wpkbd' ).' </h3> %s', $msg ) );
			}
		} else {
			wp_die( __( 'Could not find the file to download:', 'wpkbd' ) . '<br>' . esc_html( $diskfile ) );
		}
	}
	 ...
	 *
	 * By default, the folder created will be `` and will be stored
	 * inside of the `uploads` folder in WordPress' current `WP_CONTENT_DIR`,
	 * usually `wp-content/uploads`
	 *
	 * To change the folder name of `` to something else, you can use
	 * the `kbfnr_upload_dir_name` filter to change it. e.g.:
	 *
	 *     function upload_dir_name() {
	 *        return 'database-dumps';
	 *     }
	 *
	 *     add_filter( 'kbfnr_upload_dir_name', 'upload_dir_name' );
	 *
	 ...
	 * WordPress' `WP_CONTENT_DIR`, use the `kbfnr_upload_info` filter to do so. e.g.:
	 *
	 ...
	 *     }
	 *
	 *    add_filter( 'kbfnr_upload_info', 'upload_info' );
	 *
	 ...
	 *
	 * @link https://github.com/deliciousbrains/-pro-tweaks
	 *
	...
	function get_upload_info( $type = 'path' ) {
		...
		$upload_info = apply_filters( 'kbfnr_upload_info', array() );
		...
		$upload_dir_name = apply_filters( 'kbfnr_upload_dir_name', 'wpkbd' );

		if ( ! file_exists( $upload_dir['basedir'] . DIRECTORY_SEPARATOR . $upload_dir_name ) ) {
			$url = wp_nonce_url( $props->plugin_base, '-db-pro-nonce' );
