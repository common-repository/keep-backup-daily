	...
	private static function _sanitize_data( $data, $key_rules, $context, $recursion_level = 0 ) {
		...
		if ( 0 === $recursion_level && is_array( $data ) ) {
			...
			if ( ! is_array( $key_rules ) ) {
				wp_die( sprintf( '%1$s '.__('was not expecting data to be an array.', 'wpkbd' ), $context ) );
			...
			if ( 'array' == $key_rules ) {
				if ( ! is_array( $data ) ) {
					wp_die( sprintf( '%1$s '.__('was expecting an array but got something else', 'wpkbd').': "%2$s"' , $context, $data ) );

					return false;
				}
			} elseif ( 'string' == $key_rules ) {
				if ( ! is_string( $data ) ) {
					wp_die( sprintf( '%1$s '.__('was expecting a string but got something else', 'wpkbd').': "%2$s"', $context, $data ) );
				...
				if ( $key_name !== $data ) {
					wp_die( sprintf( '%1$s '.__('was expecting a valid key but got something else', 'wpkbd').': "%2$s"', $context, $data ) );
				...
				if ( $text !== $data ) {
					wp_die( sprintf(  '%1$s '.__('was expecting text but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );
				...
				if ( ! is_string( $data ) || ! is_serialized( $data ) ) {
					wp_die( sprintf( '%1$s '.__('was expecting serialized data but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );

					return false;
				}
			} elseif ( 'json_array' == $key_rules ) {
				if ( ! is_string( $data ) || ! Util\Util::is_json( $data ) ) {
					wp_die( sprintf( '%1$s '.__('was expecting JSON data but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );
				...
				if ( ! is_numeric( $data ) ) {
					wp_die( sprintf( __( '%1$s was expecting a valid numeric but got something else: "%2$s"', 'wpkbd' ), $context, $data ) );
				...
				if ( ! is_numeric( $data ) || (int) $data != $data ) {
					wp_die( sprintf( '%1$s '.__('was expecting an integer but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );
				...
				if ( ! is_numeric( $data ) || (int) $data != $data || 0 > $data ) {
					wp_die( sprintf(  '%1$s '.__('was expecting a positive number (int) but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );
				...
				if ( ! is_numeric( $data ) || (int) $data != $data || 0 < $data ) {
					wp_die( sprintf( '%1$s '.__('was expecting a negative number (int) but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );
				...
				if ( ! is_numeric( $data ) || (int) $data != $data || 0 !== $data ) {
					wp_die( sprintf( '%1$s '.__('was expecting 0 (int) but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );
				...
				if ( ! empty( $data ) ) {
					wp_die( sprintf( '%1$s '.__('was expecting an empty value but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );
				...
				if ( empty( $url ) ) {
					wp_die( sprintf( '%1$s '.__('was expecting a URL but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );
				...
				if ( empty( $bool ) || ! in_array( $bool, array( 'true', 'false' ) ) ) {
					wp_die( sprintf( '%1$s '.__('was expecting a bool but got something else', 'wpkbd' ).': "%2$s"', $context, $data ) );

					return false;
				}
				$data = $bool;
			} else {
				wp_die( sprintf( __( 'Unknown sanitization rule', 'wpkbd' ).' "%1$s" '.__('supplied by', 'wpkbd' ).' %2$s', $key_rules, $context ) );
