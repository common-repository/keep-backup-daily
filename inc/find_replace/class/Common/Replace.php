...
namespace WPMechanic\KBFNR\Common;
...
class Replace {
	...
	public function register( $args ) {
		...
		if ( ! is_array( $args ) ) {
			throw new \InvalidArgumentException( 'kbfnr_Replace '.__('constructor expects the argument to be an array', 'wpkbd') );
		}

		foreach ( $keys as $key ) {
			if ( ! isset( $args[ $key ] ) ) {
				throw new \InvalidArgumentException( "kbfnr_Replace ".__("constructor expects", 'wpkbd')." '$key' ".__("key to be present in the array argument", 'wpkbd') );
			}
		}
	...
	function is_subdomain_replaces_on() {
		if ( ! isset( $this->subdomain_replaces_on ) ) {
			$this->subdomain_replaces_on = ( is_multisite() && is_subdomain_install() && ! $this->has_same_base_domain() && apply_filters( 'kbfnr_subdomain_replace', true ) );
		}
	...
	function detect_protocol_mismatch() {
		...
		if ( 'import' !== $this->intent ) {
			$kbfnr_home_urls['remote'] = isset( $this->site_details['remote']['home_url'] ) ? $this->site_details['remote']['home_url'] : $this->site_details['remote']['site_url'];
		} else {
			...
			$kbfnr_home_urls['remote'] = $this->state_data['import_info']['protocol'] . ':' . $this->state_data['import_info']['URL'];
		}
		 ...
		 * @param array
		 */
		$kbfnr_home_urls = apply_filters( 'kbfnr_replace_site_urls', $kbfnr_home_urls );

		$local_url_is_https  = false === stripos( $kbfnr_home_urls['local'], 'https' ) ? false : true;
		$remote_url_is_https = false === stripos( $kbfnr_home_urls['remote'], 'https' ) ? false : true;
		...
		if ( 'push' === $this->intent ) {
			...
			$this->destination_url      = $kbfnr_home_urls['remote'];
		} else {
			$this->destination_protocol = $local_protocol;
			$this->source_protocol      = $remote_protocol;
			$this->destination_url      = $kbfnr_home_urls['local'];
		}
	...
	function do_protocol_replace( $new, $destination_url ) {
		 ...
		 */
		$do_protocol_replace = apply_filters( 'kbfnr_replace_destination_protocol', true, $destination_url );
	...
	function recursive_unserialize_replace( $data, $serialized = false, $parent_serialized = false, $filtered = true ) {
		$pre = apply_filters( 'kbfnr_pre_recursive_unserialize_replace', false, $data, $this );
		if ( false !== $pre ) {
			return $pre;
		}
		...
		if ( true === $filtered ) {
			list( $data, $before_fired, $successive_filter ) = apply_filters( 'kbfnr_before_replace_custom_data', array(
				$data,
				$before_fired,
				$successive_filter,
			), $this );
		...
		try {
			if ( is_string( $data ) && ( $unserialized = Util::unserialize( $data, __METHOD__ ) ) !== false ) {
				...
				if ( 'object' == gettype( $unserialized ) ) {
					...
					if ( $unserialized instanceof \__PHP_Incomplete_Class && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
						...
						$error      = sprintf( __( "- Failed to instantiate object for replacement. If the serialized object's class is defined by a plugin, you should enable that plugin for migration requests. Class Name", 'wpkbd' ).": %s", $objectName );
						error_log( $error );
				...
				list( $data, $do_replace ) = apply_filters( 'kbfnr_replace_custom_data', array( $data, true ), $this );
			...
			$error_msg     = __( 'Failed attempting to do the recursive unserialize replace. Please contact support.', 'wpkbd' );
		...
		if ( true === $filtered ) {
			$data = apply_filters( 'kbfnr_after_replace_custom_data', $data, $before_fired, $this );
		}
	...
	{
		$this->json_replace_tables = apply_filters( 'kbfnr_json_replace_tables', [
			"${prefix}posts",
		] );

		$this->json_replace_columns = apply_filters( 'kbfnr_json_replace_columns', [
			'post_content',
