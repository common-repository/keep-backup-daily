...
class Notice{
	...
	function check_notice( $notice, $dismiss = false, $reminder = false, $css_class = 'notice-link' ) {
		if ( true === apply_filters( 'kbfnr_hide_' . $notice, false ) ) {
			return false;
		}
		global $current_user;
		$notice_links = array();
		...
		if ( $dismiss ) {
			if ( get_user_meta( $current_user->ID, 'kbfnr_dismiss_' . $notice ) ) {
				return false;
			}
			$notice_links['dismiss'] = '<a href="#" class="' . esc_attr( $css_class ) . '" data-notice="' . $notice . '" data-type="dismiss">' . _x( 'Dismiss', 'dismiss notice permanently', 'wpkbd' ) . '</a>';

			if ( 'SHOW_ONCE' === $dismiss ) {
				update_user_meta( $current_user->ID, 'kbfnr_dismiss_' . $notice, true );
			}
		}

		if ( $reminder ) {
			if ( ( $reminder_set = get_user_meta( $current_user->ID, 'kbfnr_reminder_' . $notice, true ) ) ) {
				if ( strtotime( 'now' ) < $reminder_set ) {
					return false;
				}
			}
			$notice_links['reminder'] = '<a href="#"  class="' . esc_attr( $css_class ) . '" data-notice="' . $notice . '" data-type="reminder" data-reminder="' . $reminder . '">' . __( 'Remind Me Later', 'wpkbd' ) . '</a>';
		}
