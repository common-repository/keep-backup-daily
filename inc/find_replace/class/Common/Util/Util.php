...
namespace WPMechanic\KBFNR\Common\Util;

use WPMechanic\KBFNR\Common\Db\MDBWPDB;
...
use WPMechanic\KBFNR\Pro\Transfers\Files\Payload;
 ...
 *
 * @package WPMechanic\KBFNR\Common\Util
 */
class Util {
	 ...
	 * @return bool
	 */
	public static function has_method_been_called( $method ) {
	...
	/**
	 * Returns the kbfnr_bottleneck value in bytes
	 *
	 * @param string $type
	 *
	 * @return int
	...
	function get_bottleneck( $type = 'regular' ) {
		...
		$post_max_upper_size = apply_filters( 'kbfnr_post_max_upper_size', 26214400 );
		...
		return apply_filters( 'kbfnr_bottleneck', $calculated_bottleneck );
	}
	...
	/**
	 * Test to see if executing an AJAX call specific to the  family of plugins.
	 *
	 * @return bool
	 */
	public static function is_ajax() {
		...
		// must be one of our actions -- e.g. core plugin (kbfnr_*), media files (kbfnrmf_*)
		if ( ! isset( $_POST['action'] ) || 0 !== strpos( $_POST['action'], 'kbfnr' ) ) {
			return false;
		}
	...
	/**
	 * Checks if another version of kbfnr(Pro) is active and deactivates it.
	...
	public static function unserialize( $serialized_string, $method = '' ) {
		...
		if ( false === $unserialized_string && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
			$scope = $method ? sprintf( __( 'Scope', 'wpkbd' ).': %s().', $method ) : false;
			$error = sprintf( __( 'kbfnr Error: Data cannot be unserialized.', 'wpkbd' ).' %s', $scope );
			error_log( $error );
		}
	...
	/**
	 * Require kbfnr-wpdb and create new instance
	 *
	 * @return MDBWPDB
	 */
	public static function make_kbfnr_wpdb_instance() {
		return new MDBWPDB();
	}
	...
	/**
	 * Removes filters from $wp_filter that might interfere with kbfnr nonce generation/checking
	 */
	private static function filter_nonce_filters() {
		global $wp_filter;
		$filtered_filters = apply_filters( 'kbfnr_filtered_filters', array(
			'nonce_life',
		) );
	 ...
	 * @return bool
	 */
	public static function is_kbfnr_ajax_call() {
	...
	public function site_details() {
		...
		$site_details = apply_filters( 'kbfnr_site_details', $site_details );
	...
	public function get_plugin_folder( $slug ) {
		if ( isset( $GLOBALS['kbfnr_meta'][ $slug ]['folder'] ) ) {
			return $GLOBALS['kbfnr_meta'][ $slug ]['folder'];
		}
