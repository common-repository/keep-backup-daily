...
use WPMechanic\KBFNR\Common\Sanitize;
 ...
 * Class PluginManager
 *
 * @package WPMechanic\KBFNR\Common\Plugin
 */
class PluginManagerBase {
	...
	public function register() {
		...
		add_action( 'wp_ajax_kbfnr_process_notice_link', array( $this, 'ajax_process_notice_link' ) );
		add_action( 'wp_ajax_kbfnr_process_notice_link', array( $this, 'ajax_process_notice_link' ) );
		...
		if(isset($_GET['page']) && in_array($_GET['page'], array("kbd_settings", "kbd_settings_pro")))
			add_action('admin_enqueue_scripts', array($this, 'admin_head_connection_info'));
	...
	public function maybe_schema_update() {
		...
		$schema_version = get_site_option( 'kbfnr_schema_version' );
		$update_schema  = false;
		...
		if ( false === $schema_version && is_multisite() && is_network_admin() ) {
			$schema_version = get_option( 'kbfnr_schema_version' );
			if ( false !== $schema_version ) {
				update_site_option( 'kbfnr_schema_version', $schema_version );
				delete_option( 'kbfnr_schema_version' );
			}
		}

		do_action( 'kbfnr_before_schema_update', $schema_version );
		...
		if ( $schema_version < 1 ) {
			$error_log = get_option( 'kbfnr_error_log' );
			...
			if ( false !== $error_log && false === is_multisite() ) {
				...
				add_option( 'kbfnr_error_log', $error_log, '', 'no' );
			}
		...
		do_action( 'kbfnr_after_schema_update', $schema_version );
	}
	...
	function admin_head_connection_info() {
		...
		$nonces = apply_filters( 'kbfnr_nonces', array(
		...
		$data = apply_filters( 'kbfnr_data', array(
			...
			'nonces'                 => $nonces,
			'profile'                => isset( $_GET['kbfnr-profile'] ) ? $_GET['kbfnr-profile'] : '-1',
		...
		wp_localize_script( 'kbd-scripts', 'kbfnr_data', $data );
	...
	public function maybe_disable_wp_engine_filtering() {
		...
		// Make sure this is a  Ajax request
		if ( ! Util::is_ajax() ) {
			return;
		}
	...
	public function ajax_process_notice_link() {
		...
		global $current_user;
		$key   = 'kbfnr_' . $_POST['type'] . '_' . $_POST['notice'];
		$value = true;
	...
	public function get_plugin_title() {
		return $this->props->is_pro ? __( 'Migrate DB Pro', 'wpkbd' ) : __( 'Migrate DB', 'wpkbd' );
	}
}
